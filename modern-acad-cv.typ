  ...
  for key in dictionary.keys() {
    ...
    for language in multilingual.lang {
      if lang == language.at(index) {
        ...
        if "description" in subset.keys() {
          if type(subset.at("description")) == "dictionary" {
            description = eval(subset.description.at(lang),mode:"markup")
          }
        }
...
#let cv-auto-list(
  ...
  for key in dictionary.keys() {
    // Extract the year from the dictionary
    let year_value = dictionary.keys().at(index)
    if (year_value in multilingual.lang.at(lang)){
      year = multilingual.lang.at(lang).at(year_value)
    }else{
      year = year_value
    }
